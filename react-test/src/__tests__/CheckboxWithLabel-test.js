// 테스트를 하기 위해서 testing-library에서 가져옴
// cleanup : 테스트가 끝나고 DOM 마운트 해제 및 정리
// fireEvent : 이벤트를 실행
// render : 컴포넌트를 실행
import {cleanup, fireEvent, getByAltText, render} from '@testing-library/react';
// 테스트할 컴포넌트 가져옴
...
it('CheckboxWithLabel이 클릭 이후 글자가 바뀜', () => {
  ...
  // 클릭을 통해서 label의 텍스트 값이 on으로 바뀌고 off는 없어지는 것 확인
  expect(queryByLabelText(/off/i)).toBeTruthy();
});
...
it('CheckboxWithLabel 클릭확인', ()=>{
    ...
    ecpect(queryByLabelText(/선택/)).toBeTruthy
})
